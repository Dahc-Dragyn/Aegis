...
if not os.path.exists("logs"):
    os.makedirs("logs")
with open("logs/forensic_sample.txt", "w") as f:
    f.write("2026-04-13T12:00:00Z [SECURITY] AI Model Interaction Detected: gpt-4-turbo from user admin\n")

scan_output = run_aegis_scan("logs/forensic_sample.txt", "171")
# Sanitize output for safe printing on Windows terminals
safe_output = scan_output.encode('ascii', errors='ignore').decode('ascii')
print(f"Agent Action: run_aegis_scan\nResult: {safe_output[:100]}...")

print("\n2. [Forensic Analysis]: Querying the ledger for Critical/Info signals.")
ledger_data = query_compliance_ledger("Info")
if ledger_data.startswith("Error"):
    print(f"Agent Observation: {ledger_data}")
    records = []
else:
    try:
        ...
        print(f"Agent Observation: Failed to parse ledger JSON: {str(e)}\nRaw Response: {ledger_data[:200]}")
        records = []
...
if records:
    ...
    try:
        ...
        print(f"Note: Tool-level verification encountered issue: {str(e)}")
    ...
    print("Skip Steps 3 & 4: No records found in ledger.")
